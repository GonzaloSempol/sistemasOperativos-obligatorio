...
public class ManejadorArchivosGenerico {
    ...
    public static String[] leerArchivo(String nombreCompletoArchivo) {
        ...
        try {
            ...
            lineaActual = lineaActual.replaceFirst("\uFEFF", "");
            while (lineaActual != null) {
                lineaActual = lineaActual.replaceAll("\u0A00", " ").replaceAll("Â", "");
    ...
    /**
     * Lee un archivo que contenga un Integer por linea.
     *
     * @param nombreCompletoArchivo
     * @return
     */
    ...
    public static String[] randomizarLineas(String[] lineas) {
        for (int i = lineas.length - 1; i > 0; i--) {
            int idx = (int) (Math.random() * (i + 1));
            String rndStr = lineas[idx];
            lineas[idx] = lineas[i];
            lineas[i] = rndStr;
        }
        return lineas;
    ...
    public static void randomizarArchivo(String path) {
        String[] lineas = leerArchivo(path);
        lineas = randomizarLineas(lineas);
        escribirArchivo(path, lineas);
    }
}
